...
const fmtM = (n) => n >= 1_000_000 ? `${(n/1_000_000).toFixed(1)}M` : n >= 1000 ? `${(n/1000).toFixed(0)}K` : String(Math.round(n));
const timeAgo = (d) => {
...
const S = {
  ...
  badge2: (st) => {
    const m = { "Active":{bg:C.greenDim,col:C.green}, "active":{bg:C.greenDim,col:C.green}, "Completed":{bg:C.greenDim,col:C.green}, "delivered":{bg:C.greenDim,col:C.green}, "In Stock":{bg:C.greenDim,col:C.green}, "paid":{bg:C.greenDim,col:C.green}, "Processing":{bg:C.blueDim,col:C.blue}, "processing":{bg:C.blueDim,col:C.blue}, "pending_approval":{bg:C.blueDim,col:C.blue}, "Pending":{bg:"rgba(245,166,35,0.15)",col:C.accent}, "pending":{bg:"rgba(245,166,35,0.15)",col:C.accent}, "Low Stock":{bg:"rgba(245,166,35,0.15)",col:C.accent}, "Inactive":{bg:C.redDim,col:C.red}, "inactive":{bg:C.redDim,col:C.red}, "Critical":{bg:C.redDim,col:C.red}, "Out of Stock":{bg:C.redDim,col:C.red}, "cancelled":{bg:C.redDim,col:C.red}, "unpaid":{bg:C.redDim,col:C.red} };
...
function Dashboard({ token }) {
  ...
  const kpiCards = [
    { label:"Revenue This Month", value:`FRw ${fmtM(kpis.revenueThisMonth)}`, delta: kpis.revenueDelta ? `${kpis.revenueDelta>0?"+":""}${kpis.revenueDelta}% vs last month` : "No prev data", up:(kpis.revenueDelta||0)>=0, icon:"◈", color:C.accent },
    { label:"Orders This Month", value:fmt(kpis.ordersThisMonth), delta: kpis.ordersDelta ? `${kpis.ordersDelta>0?"+":""}${kpis.ordersDelta}% vs last month` : "No prev data", up:(kpis.ordersDelta||0)>=0, icon:"▦", color:C.green },
    { label:"Total Products", value:fmt(kpis.totalProducts), delta:"Registered", up:true, icon:"◫", color:C.blue },
    { label:"Low Stock Alerts", value:fmt(kpis.lowStockAlerts), delta:kpis.lowStockAlerts>0?"Needs attention":"All good", up:kpis.lowStockAlerts===0, icon:"⚠", color:C.red },
  ];
    ...
    <div>
      ...
      <div style={S.grid2}>
        <div style={S.card}>
          ...
          <table style={S.table}>
            <thead><tr><th style={S.th}>Customer</th><th style={S.th}>Branch</th><th style={S.th}>Total</th><th style={S.th}>Status</th></tr></thead>
            <tbody>
              {recentOrders.length === 0 && <tr><td colSpan={4} style={{...S.td, textAlign:"center", color:C.textMuted}}>No orders yet</td></tr>}
        ...
        <div style={S.card}>
          <div style={S.cardHeader}><div style={S.cardTitle}>Best Selling Products</div><span style={{fontSize:"11px",color:C.textMuted}}>This month</span></div>
          <div style={{padding:"8px 0"}}>
            {bestSellers.length === 0 && <div style={{padding:"20px",color:C.textMuted,fontSize:"13px",textAlign:"center"}}>No sales data yet</div>}
            {bestSellers.map((p,i) => (
      ...
      <div style={S.card}>
        <div style={S.cardHeader}><div style={S.cardTitle}>Monthly Revenue</div><span style={{fontSize:"11px",color:C.green,fontWeight:"700"}}>{kpis.revenueDelta && kpis.revenueDelta>0?`▲ +${kpis.revenueDelta}% vs last month`:""}</span></div>
  ...
  return (
    <div>
      <div style={S.sectionHeader}>
        <div><div style={S.sectionTitle}>Product Management</div><div style={{fontSize:"12px",color:C.textMuted,marginTop:"2px"}}>{products.length} products</div></div>
      ...
      {loading ? <Spinner /> : (
        <div style={S.card}>
          <table style={S.table}>
            <thead><tr><th style={S.th}>Product</th><th style={S.th}>Category</th><th style={S.th}>Stock</th><th style={S.th}>Price</th><th style={S.th}>Status</th><th style={S.th}>Actions</th></tr></thead>
            <tbody>
              {products.length===0 && <tr><td colSpan={6} style={{...S.td,textAlign:"center",color:C.textMuted}}>No products found</td></tr>}
              {products.map((p,i) => {
                ...
                return (
                  <tr key={i}>
                    <td style={S.td}>
                      <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
                        {p.imageURL && <img src={p.imageURL} alt="" style={{width:"36px",height:"36px",borderRadius:"6px",objectFit:"cover",background:C.bg}} onError={e=>e.target.style.display="none"} />}
                        <div><div style={{fontWeight:"600"}}>{p.productName}</div><div style={{fontSize:"11px",color:C.textMuted}}>{p.brandName}</div></div>
                      </div>
                    </td>
      ...
      {showModal && (
        <div style={S.modal} onClick={e=>e.target===e.currentTarget&&setShowModal(false)}>
          <div style={S.modalBox}>
            ...
            {form.imageURL && <img src={form.imageURL} alt="" style={{width:"100%",height:"140px",objectFit:"cover",borderRadius:"8px",marginBottom:"14px",background:C.bg}} onError={e=>e.target.style.display="none"} />}
...
function Inventory({ token }) {
  ...
  return (
    <div>
      <div style={S.sectionHeader}>
        <div><div style={S.sectionTitle}>Inventory Management</div><div style={{fontSize:"12px",color:C.textMuted,marginTop:"2px"}}>Real-time stock tracking</div></div>
        <div style={{display:"flex",gap:"10px"}}>
      ...
      {lowStock.length > 0 && (
        <div style={{background:C.redDim,border:`1px solid ${C.red}`,borderRadius:"10px",padding:"12px 18px",marginBottom:"20px",display:"flex",alignItems:"center",gap:"10px"}}>
          ...
          <div><span style={{fontWeight:"700",color:C.red}}>{lowStock.length} products need attention</span><span style={{color:C.textMuted,fontSize:"12px"}}> — Low stock or critical levels detected.</span></div>
        </div>
      )}
      <div style={{...S.grid4,marginBottom:"20px"}}>
        {[{label:"Total Products",value:products.length,color:C.blue},{label:"In Stock",value:inStock,color:C.green},{label:"Low Stock",value:lowCount,color:C.accent},{label:"Critical / Out",value:critical,color:C.red}].map((s,i)=>(
      ...
      {loading ? <Spinner /> : (
        <div style={S.card}>
          ...
          <table style={S.table}>
            <thead><tr><th style={S.th}>Product</th><th style={S.th}>Branch</th><th style={S.th}>Stock</th><th style={S.th}>Min Level</th><th style={S.th}>Status</th><th style={S.th}>Action</th></tr></thead>
            <tbody>
              {products.length===0 && <tr><td colSpan={6} style={{...S.td,textAlign:"center",color:C.textMuted}}>No products found</td></tr>}
              {products.map((p,i)=>{
                const st = stockStatus(p);
...
function Orders({ token }) {
  ...
  return (
    <div>
      <div style={S.sectionHeader}>
        <div><div style={S.sectionTitle}>Orders</div><div style={{fontSize:"12px",color:C.textMuted,marginTop:"2px"}}>{orders.length} orders</div></div>
      </div>
      {msg && <div style={S.alert(msg.type)}>{msg.text}</div>}
      <div style={S.card}>
        <div style={S.cardHeader}>
          <div style={{display:"flex",gap:"6px",flexWrap:"wrap"}}>
            {tabs.map(t=>(
              <button key={t} onClick={()=>setFilter(t)} style={{padding:"6px 14px",borderRadius:"6px",fontSize:"12px",fontWeight:"600",cursor:"pointer",background:filter===t?C.accent:"transparent",color:filter===t?C.bg:C.textMuted,border:`1px solid ${filter===t?C.accent:C.border}`}}>{t.charAt(0).toUpperCase()+t.slice(1)}</button>
            ))}
          </div>
        </div>
        {loading ? <Spinner /> : (
          <table style={S.table}>
            <thead><tr><th style={S.th}>Customer</th><th style={S.th}>Branch</th><th style={S.th}>Total</th><th style={S.th}>Status</th><th style={S.th}>Payment</th><th style={S.th}>Date</th><th style={S.th}>Actions</th></tr></thead>
            <tbody>
              {orders.length===0 && <tr><td colSpan={7} style={{...S.td,textAlign:"center",color:C.textMuted}}>No orders found</td></tr>}
              {orders.map((o,i)=>(
                <tr key={i}>
...
function Branches({ token }) {
  ...
  const load = useCallback(() => {
    ...
    fetch(`${API}/branches`, { headers:{Authorization:`Bearer ${token}`} })
      .then(r=>r.json()).then(d=>{ setBranches(Array.isArray(d)?d:[]); setLoading(false); })
      .catch(()=>setLoading(false));
  }, [token]);
  ...
  return (
    <div>
      <div style={S.sectionHeader}>
        <div><div style={S.sectionTitle}>Branch Management</div><div style={{fontSize:"12px",color:C.textMuted,marginTop:"2px"}}>{branches.length} branches</div></div>
        <button style={S.btn("primary")} onClick={openAdd}>+ Add Branch</button>
      </div>
      {msg && <div style={S.alert(msg.type)}>{msg.text}</div>}
      {loading ? <Spinner /> : (
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"16px"}}>
          {branches.length===0 && <div style={{...S.card,padding:"20px",color:C.textMuted,gridColumn:"1/-1",textAlign:"center"}}>No branches yet. Add your first branch.</div>}
          {branches.map((b,i)=>(
            <div key={i} style={{...S.card,padding:"20px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"14px"}}>
                <div><div style={{fontWeight:"800",fontSize:"15px",marginBottom:"3px"}}>{b.name}</div><div style={{fontSize:"12px",color:C.textMuted}}>Manager: {b.managerName||"—"}</div></div>
                <span style={S.badge2(b.status||"active")}>{b.status||"active"}</span>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"12px",marginBottom:"16px"}}>
                {[{label:"Staff",value:b.stats?.staffCount??0},{label:"Orders",value:fmt(b.stats?.orderCount??0)},{label:"Revenue",value:`FRw ${fmtM(b.stats?.totalRevenue??0)}`}].map((s,j)=>(
      ...
      {showModal && (
        <div style={S.modal} onClick={e=>e.target===e.currentTarget&&setShowModal(false)}>
          <div style={S.modalBox}>
            ...
            {[{label:"Manager Name",key:"managerName"},{label:"Manager Email",key:"managerEmail"},{label:"Location",key:"location"},{label:"Phone",key:"phone"}].map(f=>(
              <div key={f.key} style={S.formRow}>
                <label style={S.formLabel}>{f.label}</label>
...
function Users({ token }) {
  ...
  const roleDesc = { owner:"Full system access", branch_manager:"Branch-level access", sales_manager:"Sales & orders", warehouse_manager:"Inventory access", cashier:"Sales & orders only", employee:"Limited access", customer:"Customer access" };

  return (
    <div>
      <div style={S.sectionHeader}>
        <div><div style={S.sectionTitle}>Users & Roles</div><div style={{fontSize:"12px",color:C.textMuted,marginTop:"2px"}}>{users.length} team members</div></div>
      </div>
      {msg && <div style={S.alert(msg.type)}>{msg.text}</div>}
      <div style={S.grid2}>
        <div style={S.card}>
          ...
          {loading ? <Spinner /> : (
            <table style={S.table}>
              <thead><tr><th style={S.th}>Name</th><th style={S.th}>Role</th><th style={S.th}>Status</th><th style={S.th}>Actions</th></tr></thead>
              <tbody>
                {users.length===0 && <tr><td colSpan={4} style={{...S.td,textAlign:"center",color:C.textMuted}}>No users found</td></tr>}
                {users.map((u,i)=>(
                  <tr key={i}>
                    <td style={S.td}>
                      <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
                        <div style={{...S.avatar,width:"28px",height:"28px",fontSize:"11px",flexShrink:0}}>{u.name.split(" ").map(n=>n[0]).slice(0,2).join("")}</div>
                        <div><div style={{fontWeight:"600",fontSize:"13px"}}>{u.name}</div><div style={{fontSize:"11px",color:C.textMuted}}>{u.branch}</div></div>
                      </div>
                    </td>
    ...
    <div>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {Object.entries(daily.summary.byBranch).map(([branch,d],i)=>(
                  <tr key={i}><td style={{...S.td,fontWeight:"600"}}>{branch}</td><td style={S.td}>{d.orderCount}</td><td style={{...S.td,color:C.accent,fontWeight:"700"}}>FRw {fmt(d.revenue)}</td></tr>
                ))}
              </tbody>
            </table>
          )}
              ...
              <table style={S.table}>
                ...
                <tbody>
                  {monthly.topProducts.map((p,i)=>(
                    <tr key={i}><td style={{...S.td,fontWeight:"600"}}>{p.name}</td><td style={S.td}>{p.sold}</td><td style={{...S.td,color:C.accent,fontWeight:"700"}}>FRw {fmt(p.revenue)}</td></tr>
                  ))}
                </tbody>
              </table>
            </>
      ...
      {branchPerf && active==="branch" && (
        <div style={S.card}>
          ...
          <table style={S.table}>
            <thead><tr><th style={S.th}>Branch</th><th style={S.th}>Orders</th><th style={S.th}>Revenue</th><th style={S.th}>Staff</th><th style={S.th}>Low Stock</th></tr></thead>
            <tbody>
